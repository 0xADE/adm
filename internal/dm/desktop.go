...
const (
	...
	pathLastSession       = "/.cache/adm/last-session"
	pathCustomSessions    = "/etc/adm/custom-sessions/"
	pathUserCustomSession = "/.config/adm-custom-sessions/"
...
// Name returns the display name of the session entry.
func (d *desktop) Name() string {
	return d.name
}

// EnvLabel returns a short environment label (e.g. xorg / wayland).
func (d *desktop) EnvLabel() string {
	return d.env.stringify()
}

// SelectionMode returns whether the user config forces session selection.
func (d *desktop) SelectionMode() enSelection {
	return d.selection
}
...
func loadUserDesktop(homeDir string) (d *desktop, lang string) {
	homeDirConf := homeDir + "/.adm"
	confDirConf := homeDir + "/.config/adm"
