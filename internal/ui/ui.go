...
//
// The custom path also pads the tail of the line with spaces so that its
// total visible width matches what textinput.View() produces for a blurred
// field of the same Width.  Without that, the focused line is shorter than
// the blurred one and lipgloss.Place(..., Center, Center, ...) — which
// centres each line of the block against the widest line — adds extra
// left-padding to the shorter line, visibly shifting the prompt of the
// currently focused field to the right whenever focus moves between fields.
// The mismatch is only observable on terminals where this branch is taken
// (e.g. TERM=xterm or the Linux text console); on richer terminals such as
// kitty both fields render via ti.View() and are already aligned.
func renderLoginInput(ti textinput.Model, asciiCursor bool) string {
	...
	out := ti.PromptStyle.Render(ti.Prompt) +
		ti.TextStyle.Inline(true).Render(displayVal) +
		cursorGlyph

	// Match the trailing padding that textinput.View() appends after the
	// cursor when Width > 0.  See textinput.Model.View(): for a cursor at
	// end-of-value the post-cursor padding is (Width - valWidth) spaces,
	// giving a total post-prompt width of Width + 1.
	if ti.Width > 0 {
		pad := ti.Width - uniseg.StringWidth(displayVal)
		if pad > 0 {
			out += ti.TextStyle.Inline(true).Render(strings.Repeat(" ", pad))
		}
	}
	return out
}
